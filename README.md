...
This site scores **55% — Inconclusive** on its own detector.

That is not a bug and it has not been special-cased away. Three signals fire, and
two of them are fair: the stylesheet really does use a `border-left: 3px` accent
strip, and the copy really is heavy on em dashes. The guard rail then holds the
score at 55 because no *structural* signal fired — which is exactly what it is
supposed to do when the only evidence is stylistic.

A detector that exempted itself would be worth less than one that takes the hit.
